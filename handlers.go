...
// func (c* ConfigPokeAreas) getPokeAreaDetails(url string){
// 	pad := PokeAreasDetails{}
// 	results, err := http.Get(url)
// 	if err != nil {
// 		log.Printf("There was an error getting data from %s: %s", url, err)
// 		return 
// 	}
// 	decoder := json.NewDecoder(results.Body)
// 	err = decoder.Decode(&pad)
// 	if err != nil {
// 		log.Printf("There was an error decoding the results: %s",err)
// 		return 
// 	}
// 	// log.Printf("This is for area: %v", pad.Areas)
// 	for _,val := range pad.Areas{
// 		fmt.Printf("%s\n",val.Name)
// 	}
// 	c.PokeAreaInformtion = append(c.PokeAreaInformtion,pad)
	


// 	// fmt.Println("Gathering details for ", url)
